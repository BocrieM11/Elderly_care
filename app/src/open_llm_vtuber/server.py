...
class WebSocketServer:
    ...
    def __init__(self, config: Config, default_context_cache: ServiceContext = None):
        ...
        # Companion CN is the only supported frontend in this portable build.
        companion_frontend = os.path.abspath(
            os.path.join(os.getcwd(), "..", "companion_cn", "static")
        )
        if not os.path.isfile(os.path.join(companion_frontend, "index.html")):
            raise FileNotFoundError(
                f"Companion frontend was not found: {companion_frontend}"
            )

        # Mount main frontend last (as catch-all)
        self.app.mount(
            "/",
            CORSStaticFiles(directory=companion_frontend, html=True),
            name="frontend",
        )
