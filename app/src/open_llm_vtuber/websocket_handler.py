...
from .agent.input_types import ImageData, ImageSource
...
class WebSocketHandler:
    ...
    def __init__(self, default_context_cache: ServiceContext):
        ...
        self.visual_update_tasks: Dict[str, asyncio.Task] = {}
        self.pending_visual_updates: Dict[str, List[ImageData]] = {}
        self.camera_snapshot_waiters: Dict[str, Dict[str, asyncio.Future]] = {}
        self.processed_external_events: OrderedDict[str, float] = OrderedDict()
        self.pending_external_event_tasks: Dict[str, asyncio.Task] = {}
        self.external_event_lock = asyncio.Lock()
    ...
    def _conversation_is_busy(self, client_uid: str) -> bool:
        """Return whether the target client (or its group) has an active turn."""
    ...
    def _latest_connected_client(self) -> Optional[str]:
        """Choose the most recently connected UI client."""
        return next(reversed(self.client_connections), None)

    def _remember_external_event(self, event_id: str) -> None:
        """Keep a bounded, 24-hour in-memory idempotency window."""
        now = monotonic()
        cutoff = now - 24 * 60 * 60
        while self.processed_external_events:
            _, recorded_at = next(iter(self.processed_external_events.items()))
            if recorded_at >= cutoff:
                break
            self.processed_external_events.popitem(last=False)

        self.processed_external_events[event_id] = now
        while len(self.processed_external_events) > 2048:
            self.processed_external_events.popitem(last=False)

    async def trigger_external_event(self, event: dict) -> dict:
        """Trigger proactive speech on the active UI for an external event."""
        event_id = event["event_id"]

        async with self.external_event_lock:
            if event_id in self.processed_external_events:
                return {
                    "status": "duplicate",
                    "event_id": event_id,
            ...
            client_uid = self._latest_connected_client()
            if client_uid is None:
                return {
                    "status": "no_client_connected",
                    "event_id": event_id,
                    "detail": "open the OLV page before sending detection events",
                }

            self._remember_external_event(event_id)

            if self._conversation_is_busy(client_uid):
                task = asyncio.create_task(
                    self._deliver_external_event_when_idle(event, timeout=60.0)
                )
                self.pending_external_event_tasks[event_id] = task
                task.add_done_callback(
                    lambda completed, accepted_event_id=event_id: (
                        self.pending_external_event_tasks.pop(accepted_event_id, None)
                    )
                )
                logger.info(
                    f"Queued external event {event_id} while conversation is busy"
                )
                return {
                    "status": "queued",
                    "event_id": event_id,
                    "detail": "conversation is busy; event will wait for up to 60 seconds",
                }

            await self._deliver_external_event(client_uid, event)
            return {
                "status": "triggered",
                "event_id": event_id,
                "detail": "proactive conversation started",
            }

    async def _deliver_external_event_when_idle(
        self, event: dict, timeout: float
    ) -> None:
        deadline = monotonic() + timeout
        while monotonic() < deadline:
            client_uid = self._latest_connected_client()
            if client_uid and not self._conversation_is_busy(client_uid):
                await self._deliver_external_event(client_uid, event)
                return
            await asyncio.sleep(0.25)

        logger.warning(
            f"External event {event['event_id']} expired before the UI became idle"
        )

    async def _deliver_external_event(self, client_uid: str, event: dict) -> None:
        websocket = self.client_connections.get(client_uid)
        if websocket is None:
            logger.warning(
                f"External event {event['event_id']} lost its target UI connection"
            )
            return
        ...
        logger.info(
            f"Delivered external event {event['event_id']} to client {client_uid}"
        )
    ...
    def _init_message_handlers(self) -> Dict[str, Callable]:
        ...
        return {
            ...
            "camera-vision-update": self._handle_camera_vision_update,
            "camera-snapshot-response": self._handle_camera_snapshot_response,
        }
    ...
    async def handle_disconnect(self, client_uid: str) -> None:
        ...
        await self._cancel_visual_update(client_uid)
        self._cancel_camera_snapshot_waiters(client_uid)
    ...
    async def _handle_conversation_trigger(
        ...
        # User speech has priority over low-priority background visual analysis.
        await self._cancel_visual_update(client_uid)
        await handle_conversation_trigger(
            ...
            current_conversation_tasks=self.current_conversation_tasks,
            broadcast_to_group=self.broadcast_to_group,
            request_camera_snapshot=lambda: self._request_camera_snapshot(
                websocket, client_uid
            ),
        )

    async def _request_camera_snapshot(
        self, websocket: WebSocket, client_uid: str, timeout: float = 0.6
    ) -> Optional[List[dict]]:
        """Request one post-ASR camera frame, falling back on timeout."""
        turn_id = uuid4().hex
        loop = asyncio.get_running_loop()
        future = loop.create_future()
        waiters = self.camera_snapshot_waiters.setdefault(client_uid, {})
        waiters[turn_id] = future
        try:
            await websocket.send_text(
                json.dumps({"type": "request-camera-snapshot", "turn_id": turn_id})
            )
            result = await asyncio.wait_for(future, timeout=timeout)
            if result:
                logger.info(f"Received current camera snapshot for turn {turn_id[:8]}")
            return result
        except asyncio.TimeoutError:
            logger.debug(f"Camera snapshot request timed out for {client_uid}")
            return None
        except Exception as error:
            logger.warning(f"Camera snapshot request failed: {error}")
            return None
        finally:
            waiters.pop(turn_id, None)
            if not waiters:
                self.camera_snapshot_waiters.pop(client_uid, None)

    async def _handle_camera_snapshot_response(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        turn_id = data.get("turn_id")
        if not turn_id:
            return
        future = self.camera_snapshot_waiters.get(client_uid, {}).get(turn_id)
        if future is None or future.done():
            return
        image = data.get("image")
        if (
            isinstance(image, dict)
            and image.get("source") == "camera"
            and isinstance(image.get("data"), str)
            and image["data"].startswith("data:image")
        ):
            future.set_result([image])
        else:
            future.set_result(None)

    def _cancel_camera_snapshot_waiters(self, client_uid: str) -> None:
        for future in self.camera_snapshot_waiters.pop(client_uid, {}).values():
            if not future.done():
                future.cancel()

    @staticmethod
    def _parse_camera_images(raw_images: List[dict]) -> List[ImageData]:
        parsed: List[ImageData] = []
        for image in raw_images:
            if not isinstance(image, dict) or image.get("source") != "camera":
                continue
            data = image.get("data")
            ...
            parsed.append(
                ImageData(
                    ...
                    relative_time_ms=image.get("relative_time_ms"),
                    sequence_index=image.get("sequence_index"),
                    motion_score=image.get("motion_score"),
                )
            )
        return parsed

    async def _handle_camera_vision_update(
        self, websocket: WebSocket, client_uid: str, data: WSMessage
    ) -> None:
        """Queue a low-priority camera state update without blocking WebSocket IO."""
        context = self.client_contexts.get(client_uid)
        if context is None or not hasattr(context.agent_engine, "update_visual_state"):
            return

        raw_images = data.get("images") or []
        if not raw_images:
            if hasattr(context.agent_engine, "clear_visual_state"):
                context.agent_engine.clear_visual_state()
            await self._cancel_visual_update(client_uid)
            return

        existing = self.visual_update_tasks.get(client_uid)
        if existing is not None and not existing.done():
            # Latest-wins: retain only the newest sample while Qwen is busy.
            images = self._parse_camera_images(raw_images)
            if images:
                self.pending_visual_updates[client_uid] = images
            return
        ...
        task = asyncio.create_task(self._run_visual_update_queue(client_uid, images))
        self.visual_update_tasks[client_uid] = task

        def consume_result(completed: asyncio.Task) -> None:
            if self.visual_update_tasks.get(client_uid) is completed:
                self.visual_update_tasks.pop(client_uid, None)
            if completed.cancelled():
                return
            try:
                completed.result()
            except Exception as error:
                logger.warning(f"Background visual update failed: {error}")

        task.add_done_callback(consume_result)

    async def _run_visual_update_queue(
        self, client_uid: str, images: List[ImageData]
    ) -> None:
        """Process the current sample, then at most the newest queued sample."""
        current_images: Optional[List[ImageData]] = images
        while current_images:
            context = self.client_contexts.get(client_uid)
            if context is None:
                return
            await context.agent_engine.update_visual_state(current_images)
            current_images = self.pending_visual_updates.pop(client_uid, None)

    async def _cancel_visual_update(self, client_uid: str) -> None:
        self.pending_visual_updates.pop(client_uid, None)
        task = self.visual_update_tasks.pop(client_uid, None)
        ...
        try:
            ...
            logger.debug(f"Background visual update cancelled for {client_uid}")
