...
class BasicMemoryAgentConfig(I18nMixin, BaseModel):
    ...
    visual_observer_enabled: Optional[bool] = Field(
        True, alias="visual_observer_enabled"
    )
    visual_observer_current_frames: Optional[int] = Field(
        2, ge=1, le=4, alias="visual_observer_current_frames"
    )
    visual_observer_temporal_frames: Optional[int] = Field(
        6, ge=2, le=8, alias="visual_observer_temporal_frames"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "visual_observer_enabled": Description(
            en="Use a separate visual observation pass before conversation",
            zh="在聊天前启用独立的视觉观察阶段",
        ),
        "visual_observer_current_frames": Description(
            en="Frames used for current appearance questions",
            zh="当前外观类问题使用的最近画面数量",
        ),
        "visual_observer_temporal_frames": Description(
            en="Frames used for temporal action questions",
            zh="动作回顾类问题使用的时间序列画面数量",
        ),
    }
