...
class Cosyvoice2TTSConfig(I18nMixin):
    ...
    api_name: str = Field(..., alias="api_name")
    voice_cache_id: str = Field("", alias="voice_cache_id")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "api_name": Description(en="API endpoint name", zh="API 端点名称"),
        "voice_cache_id": Description(
            en="Preloaded zero-shot voice cache ID",
            zh="服务启动时预加载的零样本音色缓存 ID",
        ),
    }


class FishSpeechLocalTTSConfig(I18nMixin):
    """Configuration for the local Fish Speech 1.5 streaming service."""

    base_url: str = Field(..., alias="base_url")
    reference_id: str = Field("olv_263_oral5", alias="reference_id")
    sample_rate: int = Field(44100, alias="sample_rate")
    chunk_length: int = Field(100, alias="chunk_length")
    chunk_seconds: float = Field(0.8, alias="chunk_seconds")
    seed: int = Field(42, alias="seed")
    top_p: float = Field(0.6, alias="top_p")
    repetition_penalty: float = Field(1.2, alias="repetition_penalty")
    temperature: float = Field(0.5, alias="temperature")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "base_url": Description(
            en="URL of the local Fish Speech service", zh="本地 Fish Speech 服务地址"
        ),
        "reference_id": Description(en="Cached reference voice ID", zh="缓存音色 ID"),
        "sample_rate": Description(en="PCM sample rate", zh="PCM 采样率"),
        "chunk_length": Description(en="Semantic streaming chunk size", zh="语义流式块大小"),
        "chunk_seconds": Description(en="Browser WAV chunk duration", zh="浏览器 WAV 分块时长"),
        "seed": Description(en="Fixed sampling seed", zh="固定采样种子"),
        "top_p": Description(en="Top-p sampling", zh="Top-p 采样参数"),
...
class TTSConfig(I18nMixin):
    ...
    fish_speech_local_tts: Optional[FishSpeechLocalTTSConfig] = Field(
        None, alias="fish_speech_local_tts"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "fish_speech_local_tts": Description(
            en="Configuration for local Fish Speech TTS", zh="本地 Fish Speech TTS 配置"
        ),
    ...
    def check_tts_config(cls, values: "TTSConfig", info: ValidationInfo):
        ...
        if tts_model == "azure_tts" and values.azure_tts is not None:
            ...
            tts_model == "fish_speech_local_tts"
            and values.fish_speech_local_tts is not None
        ):
            values.fish_speech_local_tts.model_validate(
                values.fish_speech_local_tts.model_dump()
            )
