...
class FunASRRemoteConfig(I18nMixin):
    """Configuration for a remote Fun-ASR service."""

    endpoint: str = Field(
        "http://127.0.0.1:10095/transcribe", alias="endpoint"
    )
    timeout: float = Field(30.0, alias="timeout")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "endpoint": Description(
            en="Fun-ASR transcription endpoint", zh="Fun-ASR 转写服务地址"
        ),
        "timeout": Description(
...
class ASRConfig(I18nMixin):
    ...
    fun_asr_remote: Optional[FunASRRemoteConfig] = Field(
        None, alias="fun_asr_remote"
    )
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "fun_asr_remote": Description(
            en="Configuration for remote Fun-ASR", zh="远程 Fun-ASR 配置"
        ),
