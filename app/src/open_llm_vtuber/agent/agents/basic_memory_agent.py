...
from ..input_types import BatchInput, ImageData, ImageSource, TextSource
...
class BasicMemoryAgent(AgentInterface):
    ...
    def __init__(
        ...
        visual_observer_enabled: bool = True,
        visual_observer_current_frames: int = 2,
        visual_observer_temporal_frames: int = 6,
    ):
        ...
        self._visual_observer = (
            VisualObserver(
                llm=llm,
                current_frame_limit=visual_observer_current_frames,
                temporal_frame_limit=visual_observer_temporal_frames,
            )
            if visual_observer_enabled
            else None
        )
    ...
    def clone_for_session(self) -> "BasicMemoryAgent":
        """Create isolated conversation/vision state while reusing model clients."""
        cloned = copy.copy(self)
        cloned._memory = []
        cloned._interrupt_handled = False
        cloned.prompt_mode_flag = False
        cloned._json_detector = StreamJSONDetector()
        cloned._visual_observer = (
            self._visual_observer.clone_empty()
            if self._visual_observer is not None
            else None
        )
        cloned.chat = cloned._chat_function_factory()
        return cloned

    async def update_visual_state(self, images: List[ImageData]) -> bool:
        if self._visual_observer is None:
            return False
        return await self._visual_observer.update_scene(images)

    def clear_visual_state(self) -> None:
        if self._visual_observer is not None:
            self._visual_observer.clear_state()
    ...
    def _to_text_prompt(self, input_data: BatchInput) -> str:
        ...
        for text_data in input_data.texts:
            if text_data.source == TextSource.INPUT:
                ...
                message_parts.append(f"[用户分享了剪贴板内容：{text_data.content}]")

        return "\n".join(message_parts).strip()

    @staticmethod
    def _append_direct_images(
        user_content: List[Dict[str, Any]], images: List[ImageData]
    ) -> bool:
        """Compatibility path used only when the visual observer is disabled."""
        image_added = False
        for image in images:
            if isinstance(image.data, str) and image.data.startswith("data:image"):
                ...
                logger.error(
                    f"Invalid image data format: {type(image.data)}. Skipping image."
                )
        return image_added
    ...
    async def _to_messages(self, input_data: BatchInput) -> List[Dict[str, Any]]:
        ...
        if self._visual_observer is not None:
            observation = None
            visual_query_type = self._visual_observer.classify_query(
                text_prompt, input_data.images or []
            )
            if visual_query_type.value != "none" and input_data.images:
                observation = await self._visual_observer.observe_turn(
                    text_prompt, input_data.images
                )

            if visual_query_type.value != "none":
                scene_context = self._visual_observer.get_chat_context()
                if scene_context:
                    user_content.append({"type": "text", "text": scene_context})

            if observation is not None:
                user_content.append(
                    {"type": "text", "text": observation.to_chat_context()}
                )

            # Screen sharing and explicit uploaded media remain available to the
            # final chat model; camera images are represented by structured state.
            if input_data.images:
                ...
                self._append_direct_images(user_content, non_camera_images)
        elif input_data.images:
            self._append_direct_images(user_content, input_data.images)

        if text_prompt:
            if user_content and self._visual_observer is not None:
                user_content.append(
                    {
                        "type": "text",
                        "text": (
                            f"用户当前问题：{text_prompt}\n请优先直接回答这个具体问题。"
                        ),
                    }
                )
            else:
                user_content.append({"type": "text", "text": text_prompt})
        elif user_content and self._visual_observer is not None:
            user_content.append(
                {
                    "type": "text",
                    "text": "请根据内部视觉观察结果自然回应当前互动。",
                }
            )
        ...
        if user_content:
            ...
            if not skip_memory:
                memory_text = text_prompt or "[用户进行了一次无声互动]"
                self._add_message(memory_text, "user")
        else:
